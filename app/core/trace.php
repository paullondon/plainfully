...
/**
 * Trace (DB-backed) for a single pipeline run.
 *
 * Rules:
 * - OFF unless PLAINFULLY_TRACE is enabled.
 * - Viewing is ADMIN only.
...
if (!function_exists('pf_trace_ttl')) {
    /**
     * Back-compat wrapper used by older hook code.
     * TTL is handled by trace table retention policy / cleanup, not here.
     */
    function pf_trace_ttl(
        ?PDO $pdo,
        string $traceId,
        string $level,
        string $stage,
        ...
        pf_trace($pdo, $traceId, $level, $stage, $event, $message, $meta, $queueId, $checkId);
    }
}